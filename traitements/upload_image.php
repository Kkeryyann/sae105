<?php
    ...
    if ($image_error == 0) {
        ...
        if ($image_type != 'image/jpeg') {
            $affichage_erreurs .='Le fichier n\'est pas au format jpeg ou l\'extension est invalide<br>';
            $erreurs++;
        }

        // Test format du fichier avec la fonction exif_imagetype
        if (exif_imagetype($image_temporaire) != IMAGETYPE_JPEG) {
            $affichage_erreurs .='Ce fichier n\'est pas une image .jpg ou .jpeg<br>';
            $erreurs++;
        }
    } else {
    ...
    if ($image_taille > 500*1024) {
        $affichage_erreurs .='Le fichier est trop volumineux<br>';
        $erreurs++;
    }
    ...
    if ($erreurs != 0) {
        ...
        $affichage_erreurs='Téléchargement terminé avec succès';
        $erreurs++;
    }
